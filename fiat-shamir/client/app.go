...
func generateProof(keyPair *KeyPair) (*SchnorrProof, error) {
	...
	// Compute commitment
	commitment := new(big.Int).Exp(generator, nonce, prime)
	...
	// Calculate response
	privateKeyInv := new(big.Int).ModInverse(keyPair.Private, prime)
	response := new(big.Int).Mul(nonce, privateKeyInv)
